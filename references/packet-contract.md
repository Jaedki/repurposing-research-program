...
6. `candidate_seed_research`: `documents`, `rescue_strategies`, `candidates`, `exclusions`; before drug searching, the worker derives one or more cited directional rescue strategies owned by the focal primary node, without seeking a quota.
...
9. `candidate_audit`: `assessments`, `excluded_candidates`, `evidence_dispositions`; one global closed-corpus packet requires an exact partition of every reviewed candidate and cannot return new documents.
...
- The landscape packet contains the compact initial source-node index, source edges, compact
  ...
  focused searches for every open coverage gap. It applies the single coverage-saturation rule
  defined in [SKILL.md](../SKILL.md#hard-boundaries), preserves pending originals across cycles,
  and has no fixed search, paper, or proposal cap.
  ...
  of the original relevance paper. At least one relevance search must complete, and
  `coverage_checks` must classify every gap as resolved, searched-unresolved, or merged.
  `asta_call_receipts` contains one non-secret row per actual call with only logical
...
- The coverage-expansion packet contains the complete post-Asta node index, source edges, compact
  ...
  search. It calls orientation, lists and reuses or creates a workspace, inspects named searches before launch, never
  interrupts the asynchronous launch, polls the same name, inspects every ranked-result page, and
  reads selected papers in one batch. A lost launch response is recovered by inspecting before the
  same logical name is relaunched. `coverage_proposals` use the same five fields as Asta proposals and cite
  ...
  and eligibility. The single receipt records the non-secret workspace, search name
...
- Each curation concept contains `atomicity`. It is `null` for `context_only` and `exclude`. For `research`, it contains exactly `focal_abnormal_state`, `causal_level`, `biological_direction`, `compartment`, and `atomicity_rationale`, all as non-empty single strings. `focal_abnormal_state` names one source-supported changed biological variable or process. `causal_level` names the level actually supported by the claim. `biological_direction` states the observed change rather than merely “abnormal” or “dysregulated.” `compartment` gives the supported subcellular, cell-type, tissue, anatomical, or systemic context. `atomicity_rationale` explains why it is one intervention variable and distinguishes linked causes, consequences, assays, biomarkers, and outcomes. Unsupported specificity makes the concept `context_only`; do not invent state metadata to obtain `research` disposition. Keep separately supplied claims distinct when they can occur independently, could justify distinct candidate-discovery routes, occupy different causal levels or compartments, or have materially different evidence. A supported nested or downstream mechanism is not demoted merely because it belongs to a broader process. If a lone source node bundles such states without separately supported proposals, retain it as `context_only` and report the missing atomic subclaims as a gap; do not fabricate them. The concepts partition and `member_node_ids` are the sole split/merge record; `proposed_splits`, `merge_targets`, and parallel identity metadata are invalid.
...
- `established_pathology_observations` is a list of `{observation, source_ids}` objects and may be empty. It retains only sourced observations that clarify the focal pathology or its causal behaviour; workers do not invent assays, thresholds, or biomarkers.
...
At the beginning of candidate seed research, before any drug search, the worker derives one or more `rescue_strategies` from the frozen focal pathology JSON, distinct mechanisms, and graph context. It does not seek a numerical quota. Each strategy records a packet-local `strategy_key`, the focal `primary_node_id`, materially used `linked_node_ids`, the pathological and rescuable states, desired direction, mechanistic basis, ownership rationale, selected graph assertion IDs, retained pathology citations, and a route-specific search outcome and summary. `search_outcome` is `seeded` or `no_supported_seed`. A seeded strategy must link to at least one candidate; an unproductive route remains visible as `no_supported_seed` and cannot be cited by a candidate.

The primary node is the node that contains the rescued state. Linked nodes provide source-backed causal or compensatory support and are not alternative homes for the same strategy. Before submission the worker retrieves and compares every linked-node profile. It does not repeat the same state and route in reverse under a linked node; a materially different rescue objective, biological direction, or causal route is a separate strategy. Python rejects repeated strategies within a packet but does not perform semantic cross-node strategy canonicalisation. During aggregation it only assigns a stable namespace to each `(primary_node_id, strategy_key)` pair.
