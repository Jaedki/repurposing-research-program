...
Each assessment contains `source_integrity`, exactly five `component_scores`, a cited `net_assessment`, and only the cited `aliases` and `why_not` findings the auditor accepts for final output. The five components are `drug_action_confidence`, `disease_mechanism_relevance`, `mechanistic_bridge_plausibility`, `translational_feasibility`, and `evidence_robustness`. The packet carries distinct category-specific anchors: 5 is the weakest non-excluded, testable case; 10 is limited; 15 is strong with one material uncertainty; and 20 is direct, convergent support. Each score has a reason and retained `source_ids`; Python sums the values without weighting to a prioritisation score out of 100 and sorts descending by total, then candidate ID, with tied totals sharing a dense rank. The score is not a probability of efficacy.
