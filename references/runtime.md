...
`V7ProgramAdapter.execute` in `scripts/v7_production_program.py` is the complete-run composition boundary for normal experimental schema-v7 work. It requires a native v7 case container and immutable source pages plus resolver, evidence, and audit inputs (or factories), invokes the four persisted production adapters in order, drives the runtime with role-scoped packets, validates the canonical snapshot, and writes the 18 output artifacts. Identical replay is byte-stable; changed content under an existing plan or revision fails. The adapter never accepts legacy-derived containers or benchmark labels. Under V7-D006, new unspecified-schema runs default to v7. Schema v7 is an experimental research tool; outputs are not independently scientifically certified and must not be treated as clinical advice or proof of efficacy.
